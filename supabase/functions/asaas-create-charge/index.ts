...
serve(async (req) => {
  ...
  try {
    ...
    const { action, itemId, itemType, paymentMethodType, amount, installments, customerId, tenantId, displayId, externalReference, paymentId, documentNumber, name, email, phone, address, addressNumber, province, customerName, customerEmail, customerPhone } = payload;
    ...
    if (tenantId) {
      const { data: stData, error: stErr } = await supabase
        ...
        .maybeSingle();
      if (stErr) console.error('[Asaas] Erro ao buscar tenant_asaas_settings por tenant_id:', JSON.stringify(stErr));
    ...
    if (action === 'setup_webhook') {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Webhook configurado com sucesso no Asaas.' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
    }
    ...
    if (action === 'sync_payment') {
      ...
      if (!paymentId && !invoiceId && !orderId && !quoteId) throw new Error('paymentId ou invoiceId é obrigatório para sync.');
      ...
      if (paymentId) {
          ...
          if (isUUID) {
              ...
              if (invRes.data) {
                  invoiceIdToSync = invRes.data.id; currentItemType = 'INVOICE';
                  gatewayIdToSearch = invRes.data.gateway_payment_id || invRes.data.payment_gateway_id || paymentId;
              } else if (ordRes.data) {
                  invoiceIdToSync = ordRes.data.id; currentItemType = 'ORDER';
                  gatewayIdToSearch = ordRes.data.gateway_payment_id || ordRes.data.payment_gateway_id || paymentId;
              } else if (quoRes.data) {
                  invoiceIdToSync = quoRes.data.id; currentItemType = 'QUOTE';
                  gatewayIdToSearch = quoRes.data.gateway_payment_id || quoRes.data.payment_gateway_id || paymentId;
              }
          } else {
      ...
      if (paymentsToSync.length === 0 && gatewayIdToSearch.startsWith('link_')) {
          const pLinkRes = await fetch(ASAAS_API_URL + '/payments?paymentLink=' + gatewayIdToSearch + '&limit=100', { headers });
      ...
      if (paymentsToSync.length === 0 && !gatewayIdToSearch.startsWith('pay_') && gatewayIdToSearch.length > 20) {
      ...
      if (paymentsToSync.length === 0 && invoiceIdToSync) {
          try {
              ...
              if (currentItemType === 'ORDER') {
                  const { data: o } = await supabase.from('orders').select('customer_id').eq('id', invoiceIdToSync).maybeSingle();
                  custId = o?.customer_id;
              } else if (currentItemType === 'QUOTE') {
                  const { data: q } = await supabase.from('quotes').select('customer_id').eq('id', invoiceIdToSync).maybeSingle();
                  custId = q?.customer_id;
              } else {
                  const { data: i } = await supabase.from('invoices').select('customer_id').eq('id', invoiceIdToSync).maybeSingle();
                  custId = i?.customer_id;
              }

              if (custId) {
                  const { data: c } = await supabase.from('customers').select('document').eq('id', custId).maybeSingle();
                  ...
                  if (cleanDoc) {
                      ...
                      if (asaasCustData?.data && asaasCustData.data.length > 0) {
                          ...
                          const custPaymentsRes = await fetch(ASAAS_API_URL + '/payments?customer=' + asaasCustomerId + '&limit=20', { headers });
                          ...
                          if (custPaymentsData?.data && custPaymentsData.data.length > 0) {
                              const paidCustPayments = custPaymentsData.data.filter((p: any) => 
                                  p.status === 'RECEIVED' || p.status === 'CONFIRMED' || p.status === 'RECEIVED_IN_CASH' || p.status === 'ANTICIPATED' || p.status === 'AUTHORIZED'
                              );
      ...
      if (realPayId && realPayId.startsWith('pay_')) {
          const payloadToUpdate: any = { gateway_payment_id: realPayId, payment_gateway_id: realPayId };
          if (invoiceNumber) {
      ...
      if (currentItemType === 'ORDER') {
          ...
          await supabase.from('invoices').update({ status: finalStatus, paid_at: finalStatus === 'PAID' ? mainPaidDate : null }).eq('id', invoiceIdToSync);
      }
      ...
      return new Response(JSON.stringify({ success: true, newStatus: finalStatus, realPaymentId: realPayId, message: `Status no Asaas: ${statusMsg}` }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
    }
    ...
    const typeLabel = itemType === 'INVOICE' ? 'Fatura' : (itemType === 'ORDER' ? 'OS' : (itemType === 'QUOTE' ? 'Orç.' : itemType));
    ...
    if (customerId) {
       const { data: customerData } = await supabase.from('customers')
         .select('name, email, phone, whatsapp, document, address, number, neighborhood, city, state, zip_code')
         .eq('id', customerId)
         .maybeSingle();
       if (customerData) {
    ...
    Object.keys(asaasCustomerPayload).forEach(key => asaasCustomerPayload[key] === undefined && delete asaasCustomerPayload[key]);
    ...
    if (asaasCustData.data && asaasCustData.data.length > 0) {
      ...
      const createCustRes = await fetch(ASAAS_API_URL + '/customers', {
        method: 'POST', headers, body: JSON.stringify(asaasCustomerPayload)
      });
      const createCustData = await createCustRes.json();
      if (createCustData.errors) {
        throw new Error('Erro ao cadastrar cliente no Asaas: ' + createCustData.errors[0].description);
      }
    ...
    if (useCheckout) {
      ...
      const createCheckoutRes = await fetch(ASAAS_API_URL + '/checkouts', { method: 'POST', headers, body: JSON.stringify(checkoutPayload) });
    ...
    const payloadToUpdate: any = {
      gateway_payment_id: paymentData.id, payment_gateway_id: paymentData.id,
      gateway_provider: 'asaas', gateway_status: paymentData.status,
      gateway_ticket_url: hostedCheckoutUrl, gateway_pix_code: responseData.pixCopiaECola,
    };
      ...
      if (safeInstallments > 1 && paymentData.installment) {
        ...
        if (instData.data && instData.data.length > 0) {
          const insertPayload = instData.data.map((p: any) => ({
            invoice_id: itemId, installment_number: p.installmentNumber,
            total_installments: safeInstallments, amount: p.value, due_date: p.dueDate,
            status: 'PENDING', payment_method: paymentMethodType,
        ...
        await supabase.from('invoice_installments').insert([{
          invoice_id: itemId, installment_number: 1, total_installments: 1,
          amount: safeAmount, due_date: dueDateStr, status: 'PENDING',
          payment_method: paymentMethodType, gateway_payment_id: paymentData.id,
          gateway_ticket_url: hostedCheckoutUrl, tenant_id: tenantId
        }]);
      }
    }

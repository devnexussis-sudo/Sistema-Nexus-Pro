-- Migration: Technician Location History & Daily Reset
-- Creates a history table to track technician movements and auto-resets positions daily
...
-- Policy: Usuários autenticados só veem histórico do seu tenant
...
-- 🔔 Nota: Para executar reset_technician_positions_daily() automaticamente,
-- configure um cron job no Supabase Dashboard ou use pg_cron:
-- SELECT cron.schedule('reset-tech-positions', '0 0 * * *', 'SELECT reset_technician_positions_daily()');

...
export const OrderTimeline: React.FC<OrderTimelineProps> = ({ orderId }) => {
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            case 'PENDENTE': return { text: 'text-slate-600', badge: 'bg-slate-100 text-slate-700 border-slate-200' };
            case 'EM_ANDAMENTO': return { text: 'text-blue-600', badge: 'bg-blue-50 text-blue-700 border-blue-200' };
            case 'PAUSADO': return { text: 'text-amber-600', badge: 'bg-amber-50 text-amber-700 border-amber-200' };
            case 'IMPEDIDO': return { text: 'text-rose-600', badge: 'bg-rose-50 text-rose-700 border-rose-200' };
            case 'CONCLUÍDO': return { text: 'text-emerald-600', badge: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
            case 'CANCELADO': return { text: 'text-slate-600', badge: 'bg-slate-100 text-slate-600 border-slate-200' };
            default: return { text: 'text-[#1c2d4f]', badge: 'bg-slate-50 text-slate-600 border-slate-200' };
        }
    };

    return (
        <div className="space-y-6">
            <h3 className="text-sm font-semibold text-slate-900 border-l-4 border-[#1c2d4f] pl-3">Linha do Tempo</h3>
                    ...
                    <div key={event.eventId || index} className="relative pl-6 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-md shadow-slate-200/40 hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <h4 className={`text-xs font-semibold ${event.eventType === 'STATUS_CHANGED' ? getStatusStyle(event.details.new_status).text : 'text-slate-800'}`}>

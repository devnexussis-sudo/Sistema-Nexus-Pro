...
export const FinancialDashboard: React.FC<FinancialDashboardProps> = ({ orders, quotes }) => {
    const [activeTab, setActiveTab] = useState<'approved_quotes' | 'completed_orders'>('approved_quotes');
    ...
    // Estratégia: Se a OS tem "valor incluso", assumimos que ela vem de um orçamento aprovado ou tem dados financeiros
    // Por enquanto, cruzamos com orçamentos ou verificamos metadados
    const completedOrdersWithRevenue = useMemo(() => {
        return orders.filter(o => o.status === OrderStatus.COMPLETED).map(order => {
            ...
            const linkedQuote = approvedQuotes.find(q => q.linkedOrderId === order.id || q.linked_order_id === order.id);
            ...
            const value = linkedQuote?.totalValue || linkedQuote?.total_value || (order.formData as any)?.price || (order.formData as any)?.value || 0;
        ...
        }).filter(o => o.revenue > 0) // Só mostra OS que tem algum valor identificado
            .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
    }, [orders, approvedQuotes]);

    // Totais
    const totalApprovedQuotes = approvedQuotes.reduce((acc, q) => acc + (Number(q.totalValue || q.total_value) || 0), 0);
    const totalRevenueRealized = completedOrdersWithRevenue.reduce((acc, o) => acc + o.revenue, 0);
    ...
    const filteredOrders = completedOrdersWithRevenue.filter(o =>
            ...
            <div className="max-w-7xl mx-auto space-y-8 pb-20 animate-fade-in">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex bg-white/60 p-1.5 rounded-2xl border border-slate-200 backdrop-blur-sm">
                        ...
                        >
                            <FileText size={14} /> Orçamentos Aprovados
                        </button>
                        <button
                        ...
                        >
                            <CheckCircle2 size={14} /> OS Realizadas (Com Valor)
                        </button>
                    </div>
                    ...
                    <div className="relative w-full md:w-auto">
                        ...
                        <input
                            ...
                            className="pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-medium text-slate-700 w-full md:w-80 focus:outline-none focus:border-indigo-500/50 transition-all"
                        />
                    </div>
                </div>
                                    ...
                                    filteredQuotes.length > 0 ? (
                                        filteredQuotes.map(quote => (
                                            <tr key={quote.id} className="hover:bg-slate-50/80 transition-colors group">
                                                <td className="p-6">
                                                    <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg">#{quote.id.slice(0, 8)}</span>
                                                </td>
                                                <td className="p-6">
                                                    <p className="text-xs font-bold text-slate-700">{quote.customerName || quote.customer_name}</p>
                                                    <p className="text-[9px] text-slate-400">{quote.customerAddress || quote.customer_address}</p>
                                                </td>
                                                <td className="p-6">
                                                    <p className="text-xs font-bold text-slate-700">{quote.title}</p>
                                                    <p className="text-[10px] text-slate-400 truncate max-w-xs">{quote.description}</p>
                                                </td>
                                                <td className="p-6">
                                                    <div className="flex items-center gap-2 text-slate-500">
                                                        <Calendar size={14} />
                                                        <span className="text-[11px] font-bold">{formatDate(quote.created_at || quote.createdAt)}</span>
                                                    </div>
                                                </td>
                                                <td className="p-6 text-right">
                                                    <span className="text-sm font-black text-slate-900">{formatCurrency(quote.totalValue || quote.total_value || 0)}</span>
                                                </td>
                                                <td className="p-6 text-center">
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-100 text-blue-700 text-[9px] font-black uppercase tracking-wide">
                                        ...
                                        <tr>
                                            <td colSpan={6} className="p-12 text-center text-slate-400">
                                                ...
                                                <p className="text-xs font-bold uppercase">Nenhum orçamento aprovado encontrado.</p>
                                            </td>
                                        </tr>
                                    )
                                ) : (
                                    filteredOrders.length > 0 ? (
                                        filteredOrders.map(order => (
                                            <tr key={order.id} className="hover:bg-slate-50/80 transition-colors group">
                                                <td className="p-6">
                                                    <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg">OS#{order.id.slice(0, 6)}</span>
                                                    {order.linkedQuote && <div className="mt-1 text-[8px] text-slate-400">Via Orç. #{order.linkedQuote.id.slice(0, 4)}</div>}
                                                </td>
                                                <td className="p-6">
                                                    <p className="text-xs font-bold text-slate-700">{order.customerName}</p>
                                                </td>
                                                <td className="p-6">
                                                    <p className="text-xs font-bold text-slate-700">{order.title}</p>
                                                    <p className="text-[10px] text-slate-400 truncate max-w-xs">{order.description}</p>
                                                </td>
                                                <td className="p-6">
                                                    <div className="flex items-center gap-2 text-slate-500">
                                                        <Calendar size={14} />
                                                        <span className="text-[11px] font-bold">{formatDate(order.updatedAt)}</span>
                                                    </div>
                                                </td>
                                                <td className="p-6 text-right">
                                                    <span className="text-sm font-black text-emerald-600">{formatCurrency(order.revenue)}</span>
                                                </td>
                                                <td className="p-6 text-center">
                                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-700 text-[9px] font-black uppercase tracking-wide">
                                        ...
                                        <tr>
                                            <td colSpan={6} className="p-12 text-center text-slate-400">
                                                <Filter size={32} className="mx-auto mb-3 opacity-20" />
                                                <p className="text-xs font-bold uppercase">Nenhuma ordem de serviço com valor encontrada.</p>
                                            </td>
                                        </tr>
                                    )
                                )}

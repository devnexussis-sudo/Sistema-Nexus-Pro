        ...
        <div className="p-8 space-y-6 animate-fade-in flex flex-col h-full bg-slate-50/20">
                            ...
                            {filteredContracts.length > 0 ? (
                                filteredContracts.map(contract => (
                                    <tr key={contract.id} className="hover:bg-slate-50/20 border-b border-slate-50 transition-colors">
                                        <td className="px-10 py-5"><div className="flex flex-col"><span className="text-[12px] font-black uppercase italic text-indigo-600 tracking-tighter">{contract.pmocCode}</span><span className="text-[9px] font-bold text-slate-400 uppercase">{contract.title.replace('CONTRATO Master: ', '')}</span></div></td>
                                        ...
                                        <td className="px-6 py-5"><span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase italic">{contract.maintenanceDay || '1'}º</span></td>
                                        <td className="px-6 py-5">
                                            <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[8px] font-black uppercase ${contract.status === OrderStatus.CANCELED ? 'bg-red-50 text-red-500' : 'bg-emerald-50 text-emerald-600'}`}>
                                                <div className={`w-1.5 h-1.5 rounded-full ${contract.status === OrderStatus.CANCELED ? 'bg-red-500' : 'bg-emerald-500'}`} />
                                                {contract.status === OrderStatus.CANCELED ? 'Inativo' : 'Ativo'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-right pr-6"><div className="flex justify-end gap-2"><button onClick={() => { setSelectedContract(contract); setViewTab('details'); setIsViewModalOpen(true); }} className="p-2.5 bg-white text-slate-400 rounded-xl hover:text-indigo-600 shadow-sm border border-slate-100 transition-all"><Eye size={14} /></button><button onClick={() => handleOpenEdit(contract)} className="p-2.5 bg-white text-slate-400 rounded-xl hover:text-indigo-600 shadow-sm border border-slate-100 transition-all"><Edit3 size={14} /></button><button onClick={() => initToggleStatus(contract)} className={`p-2.5 bg-white rounded-xl shadow-sm border border-slate-100 transition-all ${contract.status === OrderStatus.CANCELED ? 'text-emerald-500 hover:bg-emerald-50' : 'text-red-400 hover:bg-red-50'}`}><ShieldAlert size={14} /></button></div></td>
                                    </tr>
                                ))
                            ) : (
                        ...
                        <div className="p-10">
                            {modalStep === 1 && (
                                <div className="grid grid-cols-2 gap-10">
                                    <div className="space-y-6">
                                        <div className="p-6 bg-indigo-50 border border-indigo-100 rounded-[2rem] shadow-inner"><label className="text-[9px] font-black text-indigo-400 uppercase mb-2 block tracking-widest italic">Código PMOC Pro</label><p className="text-2xl font-black text-indigo-600 uppercase italic tracking-tighter">{pmocCode}</p></div>
                                        ...
                                        <div><label className="text-[9px] font-black text-slate-400 uppercase mb-2 block">Título do Contrato</label><input type="text" value={contractTitle} onChange={e => setContractTitle(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold uppercase text-indigo-600 outline-none" /></div>
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-7 gap-1.5 p-4 bg-slate-50 border border-slate-100 rounded-[2rem]">
                                                {Array.from({ length: 31 }, (_, i) => i + 1).map(d => (
                                                    <button key={d} onClick={() => setMaintenanceDay(d)} className={`py-2 rounded-xl text-[9px] font-black transition-all ${maintenanceDay === d ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-400 border border-slate-100'}`}>{d}</button>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="bg-slate-50 rounded-[3rem] p-8 h-[550px] border border-slate-100 shadow-inner flex flex-col">
                                        ...
                                        <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                            {customerEquipments.map(equip => (<div key={equip.id} onClick={() => setSelectedEquipIds(prev => prev.includes(equip.id) ? prev.filter(id => id !== equip.id) : [...prev, equip.id])} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${selectedEquipIds.includes(equip.id) ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-100 hover:border-indigo-100'}`}><div><p className="text-[10px] font-black uppercase" style={{ color: selectedEquipIds.includes(equip.id) ? 'white' : '' }}>{equip.model}</p><p className="text-[8px] font-bold opacity-60">S/N: {equip.serialNumber}</p></div>{selectedEquipIds.includes(equip.id) && <Check size={12} />}</div>))}
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {modalStep === 2 && (
                                <div className="space-y-8 animate-fade-in">
                                    <div className="grid grid-cols-3 gap-6">
                                        ...
                                        <div className="p-6 bg-slate-50 border border-slate-100 rounded-[2.5rem] shadow-sm flex flex-col justify-center items-center">
                                            ...
                                            <div className="flex gap-2 p-1.5 bg-white rounded-2xl border border-slate-100">
                                                <button onClick={() => setIncludesParts(true)} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${includesParts ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400'}`}>SIM</button>
                                    ...
                                    <div className="bg-slate-50 border border-slate-100 rounded-[3rem] p-10 space-y-4">
                                        <div className="flex items-center gap-3"><FileSignature className="text-indigo-600" size={24} /><label className="text-[11px] font-black text-slate-900 uppercase italic tracking-tighter">Termos e Acordos do Contrato (Painel de Redação)</label></div>
                            ...
                            {modalStep === 3 && (
                                <div className="max-w-xl mx-auto py-10 space-y-8 animate-fade-in">
                                    <div className="p-8 bg-indigo-50/50 rounded-[2.5rem] border border-indigo-100 flex items-center justify-between"><div className="flex items-center gap-4"><div className={`p-4 rounded-2xl ${enableAlerts ? 'bg-indigo-600' : 'bg-slate-200'} text-white shadow-lg`}><BellRing size={32} /></div><div><h4 className="text-sm font-black text-slate-900 uppercase italic">Monitoramento</h4><p className="text-[10px] text-slate-500 font-bold uppercase">Alertas Ativos</p></div></div><button onClick={() => setEnableAlerts(!enableAlerts)} className={`w-14 h-8 rounded-full relative transition-all ${enableAlerts ? 'bg-indigo-600' : 'bg-slate-300'}`}><div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all ${enableAlerts ? 'left-7' : 'left-1'}`} /></button></div>
                                    <div className={`grid grid-cols-2 gap-6 ${enableAlerts ? 'opacity-100' : 'opacity-40 grayscale pointer-events-none'}`}><div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl"><label className="text-[8px] font-black text-slate-400 uppercase mb-4 block">Gatilho (Dias)</label><div className="grid grid-cols-3 gap-2">{[1, 3, 5, 7, 10, 15].map(d => <button key={d} onClick={() => setAlertDaysBefore(d)} className={`py-2 rounded-xl text-[10px] font-black ${alertDaysBefore === d ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'bg-white shadow-sm'}`}>{d}d</button>)}</div></div><div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl"><label className="text-[8px] font-black text-slate-400 uppercase mb-4 block">Exibição P/ Dia</label><div className="grid grid-cols-3 gap-2">{[1, 2, 3, 4, 5, 0].map(f => <button key={f} onClick={() => setAlertFrequency(f)} className={`py-2 rounded-xl text-[10px] font-black ${alertFrequency === f ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'bg-white shadow-sm'}`}>{f === 0 ? 'Off' : `${f}x`}</button>)}</div></div></div>
                                </div>
                            )}
                        </div>

                        <div className="px-10 py-8 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                            <button onClick={() => setModalStep(modalStep - 1)} disabled={modalStep === 1} className={`flex items-center gap-2 px-6 py-4 text-[10px] font-black uppercase transition-all ${modalStep === 1 ? 'opacity-0' : 'text-slate-400 hover:text-indigo-600'}`}><ArrowLeft size={16} /> Passo Anterior</button>
                            <div className="flex gap-4">
                                ...
                                {modalStep < 3 ? (
                                    <button onClick={() => setModalStep(modalStep + 1)} className="px-10 py-4 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase italic shadow-lg shadow-indigo-600/20">Próximo Passo <ArrowUpRight size={16} className="inline ml-1" /></button>
                                ) : (
            ...
            {isAuditModalOpen && (
                <div className="fixed inset-0 z-[1500] flex items-center justify-center bg-slate-900/60 backdrop-blur-xl p-6">
                    <div className="bg-white rounded-[4rem] shadow-2xl p-12 max-w-sm w-full text-center animate-fade-in-up border border-indigo-100">
                        <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-2xl mx-auto flex items-center justify-center mb-6"><MessageSquare size={32} /></div>
                        <h2 className="text-xl font-black text-slate-900 uppercase italic mb-2 tracking-tighter">Protocolo de Auditoria</h2>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-6 leading-loose underline decoration-indigo-200 underline-offset-4">Justificativa obrigatória para registrar a transação no nexus.</p>
                        ...
                        <div className="space-y-3">
                            <button onClick={handleConfirmAction} disabled={isSubmitting || !changeReason} className="w-full py-5 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-xl flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all">{isSubmitting && <Loader2 className="animate-spin" size={16} />} Confirmar Transação <ArrowUpRight size={16} /></button>
                ...
                <div className="fixed inset-0 z-[1300] flex items-center justify-center bg-slate-900/40 backdrop-blur-xl p-6">
                    <div className="bg-white rounded-[4rem] w-full max-w-6xl shadow-2xl overflow-hidden animate-fade-in-up flex h-[750px]">
                        <div className="w-80 bg-slate-50 p-10 flex flex-col border-r border-slate-100">
                            <div className="text-center mb-10"><div className="w-20 h-20 bg-indigo-600 rounded-[2.5rem] mx-auto flex items-center justify-center text-white mb-4 shadow-xl"><Briefcase size={32} /></div><p className="text-[11px] font-black text-indigo-500 uppercase">{selectedContract.pmocCode}</p></div>
                            <nav className="space-y-2 flex-1">
                                <button onClick={() => setViewTab('details')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl text-[10px] font-black uppercase transition-all ${viewTab === 'details' ? 'bg-white text-indigo-600 shadow-lg translate-x-2' : 'text-slate-400'}`}><ListFilter size={18} /> Ativos</button>
                                <button onClick={() => setViewTab('terms')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl text-[10px] font-black uppercase transition-all ${viewTab === 'terms' ? 'bg-white text-indigo-600 shadow-lg translate-x-2' : 'text-slate-400'}`}><FileSignature size={18} /> Termos</button>
                                <button onClick={() => setViewTab('history')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl text-[10px] font-black uppercase transition-all ${viewTab === 'history' ? 'bg-white text-indigo-600 shadow-lg translate-x-2' : 'text-slate-400'}`}><History size={18} /> Histórico</button>
                            </nav>
                            ...
                            {viewTab === 'details' ? (
                                <div className="space-y-10 animate-fade-in">
                                    ...
                                    <div className="grid grid-cols-4 gap-4">
                                        ...
                                        <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl shadow-sm"><h5 className="text-[8px] font-black text-slate-400 uppercase mb-2">Período</h5><p className="text-sm font-black text-indigo-600 uppercase italic">{selectedContract.periodicity}</p></div>
                                ...
                                <div className="space-y-8 animate-fade-in">
                                    <div className="flex items-center gap-4 border-b border-slate-100 pb-8"><FileSignature className="text-indigo-600" size={24} /><h4 className="text-sm font-black text-slate-900 uppercase italic tracking-tighter">Termos e Condições do Acordo</h4></div>
                                ...
                                <div className="space-y-12 animate-fade-in">
                                    <div className="flex items-center gap-4 border-b border-slate-100 pb-8"><Activity className="text-indigo-600" size={24} /><h4 className="text-sm font-black text-slate-900 uppercase italic tracking-tighter">Timeline de Auditoria Nexus</h4></div>
                                    <div className="relative border-l-2 border-slate-100 ml-6 space-y-12 pb-10">
                                        {(selectedContract.logs || []).slice().reverse().map((log: AuditLog, i: number) => (
                                            <div key={i} className="relative pl-12 animate-fade-in-up">
                                                <div className="absolute -left-[11px] top-1 w-5 h-5 bg-white border-2 border-indigo-600 rounded-full flex items-center justify-center shadow-md"><div className="w-2 h-2 bg-indigo-600 rounded-full" /></div>
                                                <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-200 shadow-sm transition-all hover:border-indigo-200">
                                                    <div className="flex justify-between items-center mb-5"><span className="px-3 py-1 bg-indigo-600 text-white text-[9px] font-black uppercase italic rounded-lg tracking-widest">{log.action.replace(/_/g, ' ')}</span><span className="text-[9px] font-bold text-slate-400 bg-white border border-slate-100 px-3 py-1 rounded-xl">{new Date(log.timestamp).toLocaleString()}</span></div>
                                                    <p className="text-[13px] font-black text-slate-900 leading-snug mb-5 uppercase italic tracking-tighter">{log.details}</p>
                                                    <div className="p-6 bg-white rounded-[2rem] border border-indigo-50 shadow-inner"><p className="text-[8px] font-black text-indigo-500 uppercase mb-2 italic">Justificativa Operacional:</p><p className="text-[12px] font-bold text-slate-600 italic leading-relaxed">"{log.reason}"</p></div>
                                                    <div className="flex items-center gap-3 mt-8 pt-6 border-t border-slate-100"><div className="w-8 h-8 bg-indigo-100 text-indigo-600 rounded-xl flex items-center justify-center shadow-sm"><UserIcon size={16} /></div><span className="text-[10px] font-black text-slate-900 uppercase italic tracking-widest">{log.user}</span></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>

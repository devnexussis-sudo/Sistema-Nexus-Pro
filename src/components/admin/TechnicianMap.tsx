...
export const TechnicianMap: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden">
            ...
            <div className="absolute top-24 right-6 z-[1000] w-64 space-y-3">
                <div className="bg-white/90 backdrop-blur-md rounded-[2.5rem] p-4 shadow-2xl border border-white/20">
                    <div className="flex items-center justify-between mb-3 px-1">
                        <div className="flex items-center gap-2">
                            <div className="p-1.5 bg-indigo-600 rounded-lg shadow-lg shadow-indigo-600/20">
                                <Navigation size={14} className="text-white" />
                            </div>
                            <h2 className="text-[9px] font-black uppercase text-slate-900 italic tracking-tighter">Radar</h2>
                        </div>
                        <div className="flex flex-col items-end">
                            <span className="text-[6px] font-black text-slate-400 uppercase tracking-widest">Ativos</span>
                            <span className="text-[10px] font-black text-indigo-600 italic leading-none">{activeTechs.length}</span>
                        </div>
                    </div>
                    ...
                    <div className="relative mb-2 px-1">
                        <input
                            ...
                            className="w-full bg-slate-100/50 border-none rounded-xl px-3 py-1.5 text-[8px] font-bold text-slate-700 outline-none focus:ring-1 focus:ring-indigo-500/20 transition-all italic placeholder:text-slate-400"
                        />
                    </div>
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                        {filteredTechs.length > 0 ? filteredTechs.map(t => (
                            <div
                                key={t.id}
                                className="flex items-center gap-2 p-2 rounded-xl bg-slate-50/30 hover:bg-white border border-transparent hover:border-slate-100 transition-all cursor-pointer group"
                                onClick={() => {
                                    // Centralizar mapa seria legal aqui
                                }}
                            >
                                <img src={t.avatar} className="w-6 h-6 rounded-lg object-cover shadow-sm" />
                                <div className="flex-1 min-w-0">
                                    <p className="text-[8px] font-black text-slate-900 uppercase truncate italic leading-none">{t.name}</p>
                                    <span className="text-[6px] font-black text-slate-400 uppercase tracking-widest mt-0.5 block">{formatLastSeen(t.last_seen)}</span>
                                </div>
                            </div>
                        )) : (
                            <div className="py-4 text-center">
                                <p className="text-[7px] font-black text-slate-300 uppercase italic">
                                    Vazio
                                </p>
                            </div>
                        )}
                ...
                {/* Legend Card - More discrete */}
                <div className="bg-white/80 backdrop-blur-md rounded-full py-2 px-4 shadow-lg border border-white/20 flex items-center justify-center gap-3">

...
async function run() {
    // Vamos buscar via dataService ou apenas a lista das colunas com service role key se eu tivesse. 
    // O public pode não ver a tabela, mas eu posso tentar uma query com RLS.
    const { data, error } = await supabase.from('orders').select('*').limit(1);
    console.log("Cols:", error ? error : Object.keys(data[0] || {}).join(', '));
}
run();

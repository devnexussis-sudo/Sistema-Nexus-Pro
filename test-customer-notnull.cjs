...
async function run() {
  ...
  const { data } = await supabase.from('orders').select('id, display_id, customer_id, customer_name').not('customer_id', 'is', null).limit(1);
  console.log('Order with customer_id:', data);
}
run();
